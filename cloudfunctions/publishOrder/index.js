...
const { CATEGORY_KEYS, CATEGORY_NAMES, SLOTS, STATUS, normalizeCity } = require('./biz')
...
exports.main = async (event) => {
  ...
  const { category, desc, photos = [], location, address, addressDetail = '', cityName, expectDate, slotKey, phone, contactName } = event

  if (!CATEGORY_KEYS.includes(category)) return bad('请选择服务类型')
  ...
  // 尽力而为:给同城有效会员师傅推订阅消息(模板ID配置在 config/app 文档,未配置则跳过)
  ...
  // 超时未完成的发送会丢(尽力而为语义不变),彻底解耦待 outbox 方案( 后续)
      ...
      if (masters.length) {
        ...
        const sends = masters.map(m =>
          cloud.openapi.subscribeMessage.send({
            ...
            data: {
              thing4: { value: CATEGORY_NAMES[category] },
